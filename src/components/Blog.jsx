...
import {
  FaFolder, FaChevronDown, FaEdit, FaEllipsisV,FaArrowUp,FaArrowDown ,
...
const toggleSubmenu = (bookmarkId) => {
  ...
  //   const retryLimit = 9999999; // Set a retry limit
  //   let retryCount = 0;

  //   // Define a function to wait for a given amount of time
  //   const wait = (ms) => new Promise(resolve => setTimeout(resolve, ms));

  //   // Define the request function that can be retried
  ...
  //         // Filter out the updated item from the current array
  //         const updatedData = filteredData.filter(item => item.query_id !== query_id);

  //         // Create the updated item, potentially merging with existing item data if necessary
  //         // Ensure response.data is structured correctly or adjust as needed
  //         const updatedItem = { ...filteredData.find(item => item.query_id === query_id), ...response.data };
  ...
  //       // Check if the error is a 504 Gateway Time-out and below the retry limit
  //       if (error.response && (error.response.status === 404 || error.response.status === 500 || error.response.status === 504) && retryCount < retryLimit) {

  //         retryCount += 1;
  //         console.log(`Retrying update... Attempt ${retryCount} due to 504 Gateway Time-out`);
  //         // toast.info(`Retrying update... Attempt ${retryCount} due to 504 Gateway Time-out`);

  //         await wait(2000 * retryCount); // Increase wait time before retrying
  //         await makeRequest(); // Retry the request
  //       } else {
  //         // Log and show error for other types of errors or if retry limit is reached
    ...
    const makeRequest = async () => {
        ...
        try {
            ...
            // if (error.response && [404, 500, 504].includes(error.response.status) && retryCount < retryLimit) {
              if (error.response && error.response.status !== 200 && retryCount < retryLimit) {
  ...
  const { handleSubmit, control, register, reset, formState: { errors } } = useForm({
    defaultValues: {
      url: projectDetails?.url || "Add URL", // Set the initial value
    },
  ...
  const onBulkSubmit = async (data) => {
    ...
    // const domainName = extractDomainName(data.url);
...
const handleBulkAction = async (action) => {
  ...
  // Clear selected rows and the selected bulk action after performing the action
  setCheckedRows([]);
  setSelectedBulkAction("");
};
const BookmarkSelector = ({ bookmarks, onSelect }) => (
  <select onChange={(e) => onSelect(e.target.value)} value="">
    <option value="">Select a bookmark</option>
    {bookmarks.map((bookmark) => (
      <option key={bookmark.id} value={bookmark.id}>{bookmark.name}</option>
    ))}
  </select>
);
...
text-white p-3 rounded transition duration-150 ease-in-out lg:text-sm text-xs font-bold"
        ...
        <div className='flex items-center space-x-[50px]'> {/* Adjust the gap as needed */}
      <div>
{/* <select
  value={selectedBulkAction}
  onChange={(e) => setSelectedBulkAction(e.target.value)}
  className="p-2 border border-gray-300 rounded shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
>
  <option value="" disabled>Bulk Actions</option>
  <option value="update">Update Query</option>
  <option value="delete">Delete Query</option>
  <option value="add">Add to Bookmark</option>

</select>

{
  showBookmarkSelector && (
    <div className="absolute z-50 mt-2 bg-white rounded-md shadow-lg w-[130px]">
  ...
  className={`ml-2 p-2 text-white bg-blue rounded`}
>
  Go
</button> */}
                ...
                <table ref={tableRef} className="min-w-full divide-y divide-gray-200 ">
                  <thead className=" bg-gray-50">
                    ...
                    >
                          ...
                          {/* <th className="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-gray-500 uppercase">
  <input
    type="checkbox"
  ...
  />
                      ...
                      <th
                        className="text-xs font-medium tracking-wider text-gray-500 uppercase ">Details</th>
...
/>
                            ...
                            <td className="relative whitespace-nowrap">
                              ...
                              {row.rank_difference != null && row.rank_difference != 0 && (
                                <span
                                  className={`absolute top-[5px] left-[55px] text-sm font-bold ${row.rank_difference > 0 ? 'text-green-500' : 'text-red-500'
                                    }`}
                                >
                            ...
                            <td className="relative whitespace-nowrap sticky-right ">
                              <div className='flex mx-auto border-l-2 h-[100%]'>
                                <Menu as="div" className="relative flex flex-col justify-center transition-all duration-300 ease-in-out p-3tex" onMouseDown={() => toggleDropdown(row.query_id)}>
                                  <Menu.Button >
                                  ...
                                  >
                                    <Menu.Items className="absolute top-[-78px] 
