...
const Services = () => {
  ...
  const services = [
    {
      id: 1,
      title: t("Membership Organisations"),
      desc: t(
  ...
  return (
    <div id="services" className="max-w-screen-2xl mx-auto md:px-14 px-4 py-16">
      <div className="text-center mx-auto my-8">
        <h2 className="text-[#4D4D4D] font-semibold text-[2.25rem]">
          {t("Our Clients")}
        </h2>
        <p className="text-[#717171]">
          {t("We have been working with some Fortune 500+ clients ")}
        </p>
      </div>
      ...
      <div className="text-center mx-auto mt-20 md:w-1/2">
        <h2 className="text-[#4D4D4D] text-[2.25rem] font-semibold mb-3 text-4xl">
          {t("Manage your entire community in a single system")}
        </h2>
        <p className="text-[#717171]">{t("Who is Nextcent suitable for?")}</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1">
        {services.map((service) => (
          <div
            key={service.id}
            className=" md:w-[300px]  md:h-80  shadow cursor-pointer  hover:border-b-8 hover:border-b-indigo-600 text-center  mx-auto transition-all duration-300 flex items-center justify-center"
          >
            <div>
              <div className="bg-[#E8F5E9] h-14 w-14 mx-auto rounded-tl-3xl rounded-br-3xl">
                <img className="-ml-4" src={service.image} />
              </div>
              <h4 className="text-2xl text-[#4D4D4D] mb-2 px-3">
                {service.title}
              </h4>
