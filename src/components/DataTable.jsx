...
const Projectranks = () => {
  ...
  const [showAbove100, setShowAbove100] = useState(false);
  const [startDate, setStartDate] = useState(new Date()); // Today's date as default
  const [endDate, setEndDate] = useState(new Date()); // You can set this to a future date by default if needed
 
  const bookmarkSelectorRef = useRef(null);
 

  // Fetch bookmarks from the Redux store
  ...
  // State to track the selected bookmark for filtering
  const [selectedBookmark, setSelectedBookmark] = useState(null);

  // Convert bookmarks to react-select options format
  ...
  const handleBookmarkChange = async (selectedOption) => {
    ...
    console.log("Selected Bookmark ID:", selectedOption.value); // Log the selected bookmark ID
    ...
    try {
      ...
      // Assuming the data needs to be filtered based on some criteria from the response
      // Update the logic below according to how your data needs to be filtered/displayed
      const rankData = favoritesData.map(favourite => favourite.rank);
      console.log(rankData);

      setFilteredData(rankData); // Assuming you want to update the table data directly with the response
      setIsFolderSelected(true); // Indicate that a folder (bookmark) is selected
    } catch (error) {
      console.error("Error fetching data for selected bookmark:", error);
      toast.error("Failed to fetch data for the selected bookmark. Please try again.");
    }
  ...
  // In the component where you manage your table data:
  useEffect(() => {
    if (selectedBookmark) {
      // Filter the table data based on the selected bookmark
      const filteredData = tableData.filter(item => item.bookmarkId === selectedBookmark.value);
      setFilteredData(filteredData);
    } else {
      // If no bookmark is selected, show all data
      setFilteredData(tableData);
    }
  }, [selectedBookmark, tableData]);
  ...
  const closeBookmarkSelector = () => setShowBookmarkSelector(false);

  useOutsideClick(bookmarksRef, () => {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (bookmarkSelectorRef.current && !bookmarkSelectorRef.current.contains(event.target)) {
        setShow_BookmarkSelector(false); // If the clicked target is outside, close the bookmark selector
      }
    }
  ...
  }, [bookmarkSelectorRef, setShow_BookmarkSelector]);
  ...
  // Adjust the filteredData based on showAbove100 before slicing
  const prePaginationRows = showAbove100 ? filteredData.filter(row => row.rank === 101) : filteredData;

  // Then slice prePaginationRows for pagination
  const currentRows = prePaginationRows.slice(indexOfFirstRow, indexOfLastRow);
  ...
  useEffect(() => {
    updateFilteredData();
  }, [startDate, endDate, searchQuery, selectedTargetUrl, selectedLocationFilter, tableData]); // Include other dependencies as needed
  ...
  const handleBulkDelete = async () => {
    ...
    // Assuming 'dispatch' is already defined using useDispatch from 'react-redux'
    try {
      // Dispatch the deleteProjects thunk with the array of selected query_ids
      await dispatch(deleteProjects(checkedRows)).unwrap();
      dispatch(fetchData({ userId, projectId }))


      // Successfully deleted all selected projects
      toast.success(`${checkedRows.length} ${checkedRows.length > 1 ? 'queries' : 'query'} deleted successfully`);

      // Clear the selection after deletion
      setCheckedRows([]);

      // Optionally, refresh the list or perform any other UI updates necessary
    } catch (error) {
  ...
  const handleUpdateButton_Click = async (event, userId, query_id, project_id) => {
    ...
    const retryLimit = 10; // Reasonable retry limit
    let retryCount = 0;
  ...
  const formattedDate = (date) => {
    if (!date) {
      return ''; // Handle undefined or null dates
    }
  ...
  const fetchAndProcessChartData = async () => {
    try {
      ...
      const processedData = data.map(item => ({
        x: new Date(item.day),
        y: parseFloat(item.average_rank.toFixed(2)), // Convert to string with two decimal places
  ...
  const requestSort = (key) => {
    ...
    const filteredDataBeforeSort = tableData.filter(item => {
        // Ensure the rank is within the specified range or if "showExceedsRank" is true, include rank 101
        return (item.rank >= filter.min && item.rank <= filter.max) || (showExceedsRank && item.rank === 101);
    });
console.log(filteredDataBeforeSort)
    const sortedData = filteredDataBeforeSort.sort((a, b) => {
        if (a[key] < b[key]) return direction === 'ascending' ? -1 : 1;
        if (a[key] > b[key]) return direction === 'ascending' ? 1 : -1;
        return 0;
    });

    // Update the filteredData state with the newly sorted and filtered data
    setFilteredData(sortedData);
    // Update the sortConfig to reflect the current sorting column and direction
  ...
  const applyFilters = () => {
    ...
    const newFilteredData = tableData.filter(item => {
      // Check if the item's rank falls within the min and max range.
      const isInRange = item.rank >= filter.min && item.rank <= filter.max;

      // Check if we should include ranks of 101 based on showExceedsRank.
      const isExceedsRank = showExceedsRank && item.rank === 101;

      // Return true if either condition is met.
      return isInRange || isExceedsRank;
    });

    console.log(newFilteredData)

  // Calculate the total number of pages after filtering
  const totalNumberOfPages = Math.ceil(newFilteredData.length / rowsPerPage);

  // Check if the current page exceeds the total number of pages after filtering
  if (currentPage > totalNumberOfPages) {
    // Set the current page to the last available page if it does, or to 1 if no pages are available
    setCurrentPage(totalNumberOfPages > 0 ? totalNumberOfPages : 1);
  } else if (currentPage === 0 && totalNumberOfPages > 0) {
    // If the current page is 0 (no page selected) and there are pages available, set to 1
    setCurrentPage(1);
  }
    setFilteredData(newFilteredData)
    setShowFilters(false);
  ...
  const resetFilters = () => {
    ...
    // Resetting bookmark filters
    setSelectedBookmark(null);
    setSelectedBookmarkId(null);
    setSelectedBookmarkName('');

    // Resetting target URL and location filters
    setSelectedTargetUrl('All Sources');
    setSelectedLocationFilter('All Locations');

    // Reset search query if you have one
    setSearchQuery('');

    // Reset custom rank filters if applicable
    setShowExceedsRank(true);
    setFilter({ min: minRank, max: maxRank });
...
${isDataLoading ? 'bg-gray-200 text-slate-800 cursor-not-allowed' : 'bg-blue text-white'}`}
      ...
      <div className="w-[100%]  p-4 mt-[2rem]">
      {/* <div className="absolute right-[35px] top-[50px] z-[1000] flex flex-row space-x-[15px]">
  ...
  <div className="flex flex-col">
    ...
    />
        ...
        <div>
          ...
          <div className='flex space-x-3  mt-[1rem] w-[100%] p-4 m-auto animate-fade-in-down'>
...
text-white p-3 rounded transition duration-150 ease-in-out lg:text-sm text-xs font-bold"
  ...
  <button 
    ...
    className="flex justify-between px-8 py-2 text-gray-500 transition duration-300 bg-white border-2 border-gray-200 rounded-lg hover:text-gray-700"
  >
    ...
    {showFilters && (
      <FaTimes 
        ...
        className="absolute text-gray-500 cursor-pointer right-2 top-3 hover:text-gray-700"
      />
    )}
    <span className={`ml-4 transform transition-transform duration-300 ${showFilters ? 'rotate-180' : ''}`}>
      ▼
    </span>
  </button>
</div>
                    ...
                    {showFilters && (
                      <div className='w-[200px] bg-white shadow-md rounded border-2 border-gray-200 absolute  right-[300px]
         ...
         p-[20px] z-[10000] '>
                        ...
                        <div className=" mt-[10px] mb-[10px] checkbox-container">
                          <label className=" checkbox-label">
                            Include 100+ Rank
                            <input
                              type="checkbox"
                              checked={showExceedsRank}
                              onChange={(e) => setShowExceedsRank(e.target.checked)}
                            />
                        ...
                        <RankRangeSlider
                          minRank={minRank}
                          maxRank={maxRank}
                          minInitialValue={filter.min}
                          maxInitialValue={filter.max}
                          onRangeChange={handleRangeChange} />
                        <button onClick={applyFilters} className=' relative w-[100%] bg-blue text-white p-2 mt-4 '>Apply</button>
                      </div>
                    )}
                  </div>
                  ...
                  <Select
                    ...
                    placeholder="Select a bookmark to filter"
                    isClearable={true}
                    isSearchable={true}
                  />
                      ...
                      {checkedRows.length > 0 && (
                        <div className='flex'>
                          ...
                          <button
                            ...
                            className={`flex items-center justify-center px-4 py-2 mr-2 border-r-2 border-gray-200 ${isUpdateInProgress ? 'cursor-not-allowed text-gray-500 ' : ''}`}
                          >
                            <FiRefreshCw className={`mr-2 ${isUpdateInProgress ? 'spinner' : ''}`} />
                            Refresh
                          </button>
...
{show_BookmarkSelector && (
  ...
  >
    ...
    className="absolute  py-1 w-[200px] z-[1000] mt-2 bg-white border border-gray-300 rounded-md ">
      {filteredBookmarks.length > 0 ? (
        <>
       
          {filteredBookmarks.map((bookmark) => (
  <div key={bookmark.id} className="flex justify-between p-2 cursor-pointer hover:bg-gray-200"
  onClick={() => {
  ...
  >
    <div className="text-sm text-left"  >
      {editingBookmarkId === bookmark.b_id ? (
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              handleRenameBookmark(bookmark.b_id, bookmarkNames[bookmark.b_id], projectId);
              setEditingBookmarkId(null); // Exit edit mode
            }
          }}
    ...
    <Menu as="div" className="relative inline-block text-left">
      ...
      <Transition as={Fragment}>
        <Menu.Items className="absolute right-0 z-10 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg w-36 ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            <Menu.Item>
              {({ active }) => (
                <button
                  onClick={() => setEditingBookmarkId(bookmark.b_id)}
                  className={`flex items-center w-full px-2 py-2 text-sm text-gray-900 group rounded-md`}
                >
            ...
            <Menu.Item>
              {({ active }) => (
                <button
                  onClick={() => handleDeleteBookmark(userId, bookmark.b_id, projectId)}
        ...
        <div className="p-2 text-sm text-center text-gray-500">No bookmarks found.</div>
      )}
                ...
                <table
                  ...
                  <thead className=" bg-gray-50">
                    ...
                    >
                      ...
                      <th className="w-[15%] keywords-head text-xs font-medium tracking-wider text-gray-500 uppercase cursor-pointer " onClick={() => requestSort('query')}>
                        ...
                        <>
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              {typeof row.query === 'string' && typeof searchQuery === 'string' && (searchQuery === "" || row.query.includes(searchQuery)) ? (
                                <button onClick={(event) => handleToggleRow(row.query_id, event)} className="focus:outline-none">
                                  {expandedRowId === row.query_id ? (
                                    <FaChevronUp className="text-xl text-blue" />
                                  ) : (
                                    <FaChevronDown className="text-xl text-blue" />
                                  )}
                                </button>
                              ) : null}
                            </td>
                            ...
                            <td className='flex items-center space-x-2'>
                              {editingQueryId === row.query_id ? (
                                ...
                                <>
                                  <div
                                    className="cursor-pointer tooltipcontainer"
                                    onDoubleClick={() => handleEditClick(row.query_id, row.query)}
                                  >
   {row.new_query && row.new_query.length > 15 ? (

<>
     <span className="tooltiptext">{row.new_query}
     
     </span>
     <br/>
     <span >{row.new_query.substring(0, 15)}...</span>
     <span>   {row.google_domain}</span>
</>
) : (
<>
<br/>
...
<span>{row.google_domain}</span>
                                    ...
                                    {
                                      (searchQuery === "" || (row.new_query && row.new_query.includes(searchQuery))) ? (
                                        <button
                                          onClick={(event) => {
                                            event.stopPropagation(); // Prevent the click from propagating to higher elements
                                            copyToClipboard(row.new_query);
                                            setCopiedRows((prev) => new Set(prev.add(row.query_id))); // Indicate that the row's query has been copied
                                          }}
                                          title="Copy to clipboard"
                                          className="p-1"
                                        >
                                          {copiedRows.has(row.query_id) ? (
                                            <FaCopy className="cursor-pointer" /> // Icon indicating content has been copied
                                          ) : (
                                            <FaRegCopy className="cursor-pointer" /> // Default copy icon
                                          )}
                                        </button>
                                      ) : null // Do not render the button if conditions are not met
                                    }
                             ...
                             <td className="relative whitespace-nowrap">
                              ...
                              {row.rank !== 101 &&  row.rank_difference != null && row.rank_difference != 0 && (
                                <span
                                  ...
                                  (() => {
                                    ...
                                    return lastSegment ? (
                                      <div className="tooltipcontainer">
                                        <span className="tooltiptext">{decodeURIComponent(row.source_url)}</span>
                                        <br />
