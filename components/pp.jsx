...
const truncateText = (text, maxLength = 15) => {
  ...
  const handleRowClick = (query_id) => {
    setExpandedRowId(prev => (prev === query_id ? null : query_id));
  };
...
console.log(response)
  ...
  const handleRowInteraction = (row, event, rowIndex) => {
    if (event.target.closest('.checkbox-cell')) {
      // Prevent the event from propagating to the row's click handler
      event.stopPropagation();
      // No further action is needed here for clicks on the checkbox cell
      return;
    }
