...
const Projectranks = () => {
  ...
  const addQueryToBookmark = async (queryId, bookmarkId) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/favourites/${userId}/${queryId}/${bookmarkId}/${projectId}/`);
      if (response.status === 200) {
    ...
    setFilteredData(currentData => {
      return currentData.map(item => {
        if (item.temp && item.query === query) {
          return {
            ...item,
            first_five_links:rankData.first_five_links,
            project:rankData.project,
            rank_abs:rankData.rank_abs,
            breadcrumb:rankData.breadcrumb,
            Type_rank:rankData.Type_rank,
            result_stat:rankData.rankData,
            query_id: rankData.query_id, // Replace with actual ID
            rank: rankData.rank,
...
const [queryNameError, setQueryNameError] = useState("");
  const handleSaveEdit = async (queryId) => {
    const updatedData = { query_name: editedQueryName };
    // Assuming you have a function to update the query
    await handleUpdateQuery(queryId, updatedData);
    setEditingQueryId(null); // Reset edit mode
    setEditedQueryName(""); // Clear temporary state
  };
  ...
  // Step 3: Function to submit the new query name
  const submitNewQueryName = async (queryId) => {
    ...
    try {
      ...
      // Update the UI to reflect the new query name...
      toast.success('Query updated successfully');
...
${isDataLoading ? 'bg-gray-200 text-slate-800 cursor-not-allowed' : 'bg-blue text-white'}`}
  ...
  animate-fade-in-down'>
        ...
        <div className="flex mb-4 -mx-2">
          ...
          <div className="p-4 w-full md:w-[28.33%] px-2 z-[10000] z-[-200]">
...
text-white p-3 rounded transition duration-150 ease-in-out lg:text-sm text-xs font-bold"
        ...
        {loading ? (
          ...
          <div className="flex justify-between p-0 mt-4 border-t-2 border-gray-200 spac lg-plus:p-4">
            ...
            <div className='flex  items-center space-x-[10px] mb-[20px]'> {/* Adjust the gap as needed */}
          <div>
...
{/* <Select
  ...
  isSearchable={false}
  isDisabled={isUpdateInProgress || isUpdating} // Disable based on update states
/> */}
                    ...
                    {currentRows.length > 0 ? (
                      currentRows.map((row, index) => (
                        <>
                          ...
                          className={`${
                            selectedRows.some(selectedRow => selectedRow.query_id === row.query_id) ||
...
<td className="flex justify-center px-6 py-4 space-x-2 whitespace-nowrap">
  {editingQueryId === row.query_id ? (
    <input
          ...
          onBlur={() => submitNewQueryName(row.query_id)} // Submit on blur or you could use a separate button
          onKeyPress={(e) => e.key === 'Enter' && submitNewQueryName(row.query_id)}
          className="form-input"
        />
      ) : (
        <div onDoubleClick={() => handleEditClick(row.query_id, row.query)}>
          {row.query}
          <button onClick={() => handleEditClick(row.query_id, row.query)}>Edit</button>
        </div>
      )
      }
...
<>
      ...
      <span onDoubleClick={() => {
        setEditingQueryId(row.query_id);
        setEditedQueryName(row.query); // Initialize with current query name
      }}>
        {row.query.length > 15 ? `${row.query.substring(0, 15)}...` : row.query}
      </span>
    </>
...
<td className="flex items-center justify-center px-6 py-4 whitespace-nowrap">
  ...
  <button
    ...
    title="Copy to clipboard"
    className="p-1 mr-2" // Add some padding and margin to space it out
  >
    {copiedRows.has(row.query_id) ? (
  ...
  {editingQueryId === row.query_id ? (
    ...
    value={tempQueryName}
    onChange={handleQueryNameChange}
    onBlur={() => submitNewQueryName(row.query_id)} // Consider using a dedicated "Save" button as well
    onKeyPress={(e) => e.key === 'Enter' && submitNewQueryName(row.query_id)}
    className="form-input"
    autoFocus
  />
  ) : (
    // Display query name with Edit and ellipsis for longer text
    <div 
      className="flex-1" // Flex-1 to take up available space
      onDoubleClick={() => handleEditClick(row.query_id, row.query)} // Double click to edit
    >
      <span 
        className="cursor-pointer" 
        title={row.query} // Show full query on hover
      >
        {row.query.length > 15 ? `${row.query.substring(0, 15)}...` : row.new_query
}
      </span>
      {/* Edit button */}
      {/* <button
      
        className="ml-2 text-sm" // Margin left to space it out from the query name
      >
        Edit
      </button> */}
    </div>
                            ...
                            <td className="relative whitespace-nowrap sticky-right ">
                              <div className='flex mx-auto border-l-2 h-[100%]'>
                                <Menu as="div" className="relative flex flex-col justify-center transition-all duration-300 ease-in-out p-3tex"
                                  ...
                                  <Menu.Button >
                                    <BiDotsVertical size={18} className='p-1 m-3 rounded-full cursor-pointer hover:bg-gray-300 ' />
                                  </Menu.Button>
...
<div className=''>
  <span className='font-bold'>
    Total Rows:
  </span> {filteredData.length} {/* Adjusted to use filteredData */}
</div>
                </div>
